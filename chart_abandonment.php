...
function prod_display($email, $conn){
        ...
        while($display = $result_display->fetch_object()){
                $prod_row .= "<tr>
                    ...
                    <td class='mcnTextContent' align='center' valign='top'>$".$display->Price."</td>
                </tr>";
        }
    return $prod_row;
}

function button_link($email){
   return '<td align="center" valign="middle" class="mcnButtonContent" style="font-family: Verdana, Geneva, sans-serif; font-size: 16px; padding: 10px;"><a class="mcnButton " title="Your Cart" href="https://www.pmimd.com/onlinetraining/yourcart.php?email='.$email.'" target="_blank" style="font-weight: normal;letter-spacing: normal;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;">Your Cart</a></td>';
}

$conn = Connect();
//get email from cart with in 24hrs
$select_email ="SELECT Distinct Email FROM cart WHERE DATE(Date) = DATE_SUB(DATE(CURRENT_DATE()), INTERVAL 1 DAY ) AND Email !='1'";
